...
class ValueHead(nnx.Module):
    def __init__(
        ...
        *,
        ...
        if activation.lower() == "gelu":
            self._act_fn = nnx.gelu
        elif activation.lower() == "relu":
            self._act_fn = jax.nn.relu
        else:
        ...
        for i in range(num_layers):
            ...
            use_bias = True if i < num_layers - 1 or bias_last else False
            linear = nnx.Linear(dims[i], dims[i+1], use_bias=use_bias, rngs=rngs)
    ...
    def __call__(self, x):
        for i in range(self.num_layers):
            linear = getattr(self, f"layer_{i}")
            x = linear(x)
