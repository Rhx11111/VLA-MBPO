...
class Bagel(PreTrainedModel):
    ...
    def generate_image(
        ...
        # for i, t in tqdm(enumerate(timesteps), total=len(timesteps), desc="Generating image"):
        ...
        if enable_taylorseer:
            ...
            self.language_model.model.enable_taylorseer = False
            if hasattr(self.language_model.model, 'cache_dic'):
                delattr(self.language_model.model, 'cache_dic')
            if hasattr(self.language_model.model, 'current'):
                delattr(self.language_model.model, 'current')
            # Clean up each decoder layer
            for decoder_layer in self.language_model.model.layers:
                if hasattr(decoder_layer, 'enable_taylorseer'):
                    decoder_layer.enable_taylorseer = False
                if hasattr(decoder_layer, 'cache_dic'):
                    delattr(decoder_layer, 'cache_dic')
                if hasattr(decoder_layer, 'current'):
                    delattr(decoder_layer, 'current')
