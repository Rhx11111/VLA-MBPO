...
def main():
    ...
    training_args.results_dir = os.path.join(training_args.results_dir, training_args.wandb_project, f"{training_args.wandb_name}-run{training_args.wandb_runid}")
    training_args.checkpoint_dir = os.path.join(training_args.results_dir, "ckpt")
    ...
    if training_args.copy_init_moe:
        logger.info('Init MOE...')
        language_model.init_moe()

    if training_args.visual_und:
        logger.info('Preparing Siglip...')
        if training_args.finetune_from_hf:
    ...
    # logger.info('Preparing EMA model...')
    ...
    if resume_model_only:
        ...
        if resume_from is not None and os.path.exists(resume_from):
            try:
                train_step = int(os.path.basename(os.path.normpath(resume_from))) + 1
            except:
                train_step = 0
        else:
            train_step = 0
    ...
    # if train_step > 0:
    #     skipped = 0
    #     logger.info(f"Resuming dataloader by skipping {train_step} steps...")
    #     for _ in tqdm(range(train_step), desc="Skipping dataloader"):
    #         try:
    #             next(train_iter)    
    #             skipped += 1
    #         except StopIteration:
    #             logger.warning(
    #                 f"Dataloader exhausted after skipping {skipped} steps; restarting iterator."
    #             )
    #             train_iter = iter(train_loader)
    #             break
    #     dist.barrier()
    ...
    for curr_step, data in enumerate(train_loader, start=train_step):
        ...
        if curr_step > 0 and curr_step % training_args.save_every == 0:
            # Clear CUDA cache before checkpoint saving to avoid OOM
            torch.cuda.empty_cache()
            # Synchronize all processes before saving
            dist.barrier()
            
            # Skip gathering data_status since it's not saved anyway
            # This avoids potential NCCL errors from large object transfers
            FSDPCheckpoint.fsdp_save_ckpt(
                ckpt_dir=training_args.checkpoint_dir, 
                train_steps=curr_step, 
                model=fsdp_model, 
                ema_model=None, 
                optimizer=None,  # Not saving optimizer 
                scheduler=None,  # Not saving scheduler
                logger=logger,
                fsdp_config=fsdp_config,
                data_status=None      # Not saving data_status
            )
        
        if curr_step > training_args.total_steps:
            logger.info('Reach max training steps, break')
            break
